...
export default (function ($) {
      ...
      return this.each(function () {
        ...
        for (var i in sum_array) {
          ...
          if (mult_array.length > 1) {
            ...
            sum =
              sum +
              (!isNaN(parseFloat($("#" + sum_array[i]).val()))
                ? parseFloat($("#" + sum_array[i]).val())
                : !isNaN(parseFloat(sum_array[i]))
                ? parseFloat(sum_array[i])
                : 0);
        }
        var regex = new RegExp(
          "\\d(?=(\\d{3})+\\" + options.decimalSeparator + ")",
          "g"
        );
        $(this).html(
          (options.currencyPosition == "before"
            ? '<span class="currency">' + options.currency + "</span>"
            : "") +
            sum
              .toFixed(2)
              .replace(".", options.decimalSeparator)
              .replace(regex, "$&" + options.thousandthSeparator) +
            (options.currencyPosition == "after"
              ? '<span class="currency">' + options.currency + "</span>"
              : "")
        );
        if (jQuery.type(options.updateHidden) == "object")
          options.updateHidden.val(
            options.currency +
              sum
                .toFixed(2)
                .replace(".", options.decimalSeparator)
  ...
  jQuery.fn.costCalculator = function (method) {
    if (methods[method]) return methods[method].apply(this, arguments);
